...
export const createCashOrder = asyncHandler(async (req, res, next) => {
  ...
  const cartPrice = cart.totalPriceAfterCouponDiscount || cart.totalPrice;
  ...
  if (order) {
    // Giảm stock variant và cập nhật tổng quantityInStock và sold
    for (const item of cart.cartItems) {
      ...
      product.quantityInStock = product.variants.reduce(
        (sum, v) => sum + v.stock,
        0
      );
    ...
    line_items: [
      {
        price_data: {
          ...
          product_data: {
            ...
            description: `Your Cart Is Ready To Payment Process ... Click Pay Now To Place Order`,
            images: [
